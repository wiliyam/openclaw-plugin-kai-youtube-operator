...
function sanitizeError(data: Record<string, unknown>): string {
...
async function refreshAccessTokenIfNeeded(): Promise<StoredToken> {
...
function queryString(query: QueryParams): string {
...
async function youtubeAnalyticsRequest<T>(query: QueryParams): Promise<T> {
  return authorizedJsonRequest<T>(YOUTUBE_ANALYTICS_API_URL, "GET", "reports", query);
}

async function youtubeMediaUploadRequest<T>(params: {
  method: "POST" | "PUT";
  resourcePath: string;
  query?: QueryParams;
  filePath: string;
...
async function youtubeMultipartUploadRequest<T>(params: {
  method: "POST" | "PUT";
  resourcePath: string;
  query?: QueryParams;
  metadata: unknown;
...
function limitOutput(value: string, maxCharacters = 12_000): string {
  return value.length > maxCharacters ? value.slice(-maxCharacters) : value;
}

function runCommand(command: string, args: string[], timeoutMs = 15 * 60_000): Promise<CommandResult> {
...
async function prepareOutputFile(outputPath: string, overwrite: boolean | undefined): Promise<void> {
...
function validateShortDuration(durationSeconds: number): void {
...
function aspectFilter(aspect: ShortAspect, fit: ShortFit): string | null {
...
async function createShortFromVideo(params: {
  ...
  if (params.captionFilePath) await assertFileExists(params.captionFilePath);
  if (params.fontFile) await assertFileExists(params.fontFile);
...
function defaultShortOutputPath(inputPath: string): string {
  const parsed = path.parse(inputPath);
  const suffix = randomBytes(4).toString("hex");
  return path.join(DEFAULT_SHORTS_DIR, `${parsed.name}-short-${suffix}.mp4`);
}

function safeFfmpegColor(value: string | undefined, fallback: string): string {
...
async function generateThumbnailCard(params: {
  ...
  if (params.inputPath) await assertFileExists(params.inputPath);
  if (params.fontFile) await assertFileExists(params.fontFile);
  await prepareOutputFile(params.outputPath, params.overwrite);
...
function generatedAudioFilter(style: GeneratedAudioStyle, durationSeconds: number): string {
  const duration = formatSeconds(durationSeconds);
  if (style === "silence") return `anullsrc=channel_layout=stereo:sample_rate=44100:d=${duration}`;
  if (style === "white_noise") return `anoisesrc=color=white:duration=${duration}:amplitude=0.05`;
  if (style === "clean_tone") return `sine=frequency=440:duration=${duration}:sample_rate=44100`;
  if (style === "soft_pulse") {
...
function mergeBroadcastUpdate(existing: Record<string, unknown>, params: {
  title?: string;
  description?: string;
  scheduledStartTime?: string;
  scheduledEndTime?: string;
...
function buildChannelBrandingUpdateBody(existing: Record<string, unknown>, params: {
  title?: string;
  description?: string;
  keywords?: string;
  country?: string;
...
function mergePlaylistUpdate(existing: Record<string, unknown>, params: {
  title?: string;
  description?: string;
  privacyStatus?: PrivacyStatus;
  tags?: string[];
...
async function getPlaylistItemById(id: string) {
...
function buildPlaylistItemUpdate(existing: Record<string, unknown>, params: {
  playlistId?: string;
  videoId?: string;
  position?: number;
  note?: string;
...
async function getLiveStreamById(id: string) {
...
function mergeLiveStreamUpdate(existing: Record<string, unknown>, params: {
  title?: string;
  description?: string;
}) {
  const snippet = existing.snippet as Record<string, unknown> | undefined;
